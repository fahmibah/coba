...
class NostalgiaForInfinityNext(IStrategy):
   ...
   def sell_under_mainn(self, current_profit: float, last_candle) -> tuple:
        if last_candle['close'] < last_candle['ema_200']:
    ...
    def sell_over_mainn(self, current_profit: float, last_candle) -> tuple:
        if last_candle['close'] > last_candle['ema_200']:
    ...
    def custom_sell(self, pair: str, trade: 'Trade', current_time: 'datetime', current_rate: float,
        ...
        # Over EMA200, main profit targets
        sell, signal_name = self.sell_over_mainn(current_profit, last_candle)
        if sell and (signal_name is not None):
            return f"{signal_name} ( {buy_tag} )"
        ...
        # Under EMA200, main profit targets
        sell, signal_name = self.sell_under_mainn(current_profit, last_candle)
        if sell and (signal_name is not None):
            return f"{signal_name} ( {buy_tag} )"
